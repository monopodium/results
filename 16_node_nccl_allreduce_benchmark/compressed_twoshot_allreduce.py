...
class CompressedNosizeAllReduce:
    ...
    def __init__(self, max_elements, device, compress_rs=True, compress_ag=True):
        ...
        # Pure naive path: nothing is compressed, skip all dietgpu buffers
        # (including the multi-GB temp workspace) to keep memory lean.
        self._pure_naive = (not compress_rs) and (not compress_ag)

        chunk = max_elements // ws

        if not self._pure_naive:
            ...
            # Pure naive: only need the uncompressed RS recv buffer.
            self.max_comp_cols = 0
            self.max_fb = 0

        # Raw RS recv buffer (used by uncompressed RS path and pure-naive path)
    ...
    def __call__(self, input_tensor):
        ...
        # ── Pure-naive fast path: no compression at all ──
        if self._pure_naive:
            return self._call_naive(input_tensor, C)
    ...
    # ─────────────────────── pure-naive fast path ───────────────────────

    def _call_naive(self, input_tensor, C):
        """Two-shot allreduce with no compression anywhere — same batch
        all-to-all pattern as the compressed path but with raw bf16 chunks.

        RS:  all-to-all exchange into rs_raw_recv, fused sum into my shard.
        AG:  all-to-all exchange, recv directly into final positions
             (no temporary buffer, no copy)."""
        rank = self.rank
        ws = self.ws
        npeers = self.npeers
        ...
        my_start = rank * C
        if npeers == 1:
            input_tensor[my_start:my_start + C].add_(self.rs_raw_recv[:C])
        else:
            input_tensor[my_start:my_start + C].add_(
                self.rs_raw_recv[:npeers * C].view(npeers, C).sum(dim=0))
        ...
        for peer in range(ws):
            if peer == rank:
                continue
            ops.append(dist.P2POp(dist.isend, my_reduced, peer))
            # Recv directly into the final position — no staging buffer
            ops.append(dist.P2POp(dist.irecv,
                       input_tensor[peer * C:(peer + 1) * C], peer))
        for r in dist.batch_isend_irecv(ops):
            r.wait()
...
def naive_twoshot_allreduce(input_tensor):
    """Pure naive two-shot allreduce — no compression anywhere.

    Same all-to-all RS + AG pattern as the compressed path but sends raw
    bf16 chunks. Shares the (False, False) instance slot with
    `compressed_nosize_allreduce(..., compress_rs=False, compress_ag=False)`;
    because both flags are off, no dietgpu buffers or multi-GB temp workspace
    are allocated."""
    return compressed_nosize_allreduce(input_tensor,
                                       compress_rs=False,
                                       compress_ag=False)
